...
const getAllExercises = async (req, res) => {
  ...
  const filter = (await exerciseService.getFilterByOptions({ name: type }))[0]
    ?.filter;
  if (!filter) {
  ...
  const exercises = await exerciseService.getExerciseByOptions(
...
const getAlldetails = async (req, res) => {
  const equipmentList = await exerciseService.getFilterByOptions({
    filter: "Equipment",
  });
  const bodyPartsList = await exerciseService.getFilterByOptions({
    filter: "Body parts",
  });
  const musculesList = await exerciseService.getFilterByOptions({
    filter: "Muscles",
  });
